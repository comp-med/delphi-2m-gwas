...
## Script to run the second step of REGENIE on the male-only embeddings
#SBATCH --job-name=emb120_male_s2
...
#! set name
#SBATCH --output=slurm-%x-%j-step2-emb-male.out
...
export step1_output=${current_proj}/gwas/input/emb120_male
export output=${current_proj}/gwas/output_sex_stratified/emb120_male
export tmpdir=${current_proj}/gwas/tmpdir/emb120_male
...
## Do some logging
pheno=male_norm_embeds_phenotype.txt
...
--keep ${sample_qc} \
--phenoFile ${current_proj}/gwas/data/male_norm_embeds_phenotype.txt \
...
--qt \
--gz \
--pred ${step1_output}/ukb_emb_male_step1_pred.list \
--bsize 400 \
--out ${output}/gwas_emb120_male_chr${chr}
