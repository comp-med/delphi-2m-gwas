...
## Script to run the second step of REGENIE on the individual-level's embeddings
#SBATCH --job-name=emb120_s2
...
export step1_output=${current_proj}/gwas/input/emb120
export output=${current_proj}/gwas/output/emb120
export tmpdir=${current_proj}/gwas/tmpdir/emb120
...
## Do some logging
pheno=norm_embeds_phenotype_441189.txt
...
## old version:
#cat ${var}/ukb_imp_EUR_chr${chr}_snpstat.out | awk -v chr=${chr} '{if(NR != 1 && $3 == chr) print $2}' - > ${tmpdir}/tmp.ex.chr${chr}.list
## new:
...
--keep ${sample_qc} \
--phenoFile ${current_proj}/gwas/data/norm_embeds_phenotype_441189.txt \
...
--qt \
--gz \
--pred ${step1_output}/ukb_emb_step1_pred.list \
--bsize 400 \
--out ${output}/gwas_emb120_chr${chr}
